...
<p class="back_height"><a href="index.php"><span class="back"><<< </span>Back to the Player Finder</a></p>
	...
	<h2>HaloFinder is for Halo 1 ONLY.</H2>
	<p class="call_action">Please note that you will have to verify your email before you appear on the player finder. The verification email may go to your spam folder.
	<p class="call_action">Please whitelist no-reply@halo1hub.com in your email software since system and player emails will both come from that.
<h2 id="version">Step 1 - Location</h2>
<div id="location" class="row">
<div id="form" class="block">
<p class="call_action">Enter your location here, then click "Find Location."</p><br>
    <ul>
...
<p class="call_action">Fill out your player information. 
...
<textarea name="notes" id="pnotes" rows="5" cols="80" maxlength=160 placeholder="Any locals? Need equipment? Are you so bad people should know?" autocorrect=off style="width: 390px;"></textarea><br/><br />
  ...
  <div class="g-recaptcha" data-theme="dark" data-sitekey="6Le_04wUAAAAAF3lM7Q5TdrQ6Z6Pj8BMfAI13Ccb"></div>
	 <div class="button_cont" align="left"><input class="example_b" type="submit" name="submit" value="ADD PLAYER" />
        </div>
</form>
</div>
 <script>
	...
	if(checkCaptcha()){
		...
		if($lat and $lng)
		{
			if($email == $verifyemail)
			{
				...
				if (!($sql = $conn->prepare("INSERT INTO FINDER.PLAYERS (NAME,LAT,LNG,CITY,EMAIL,GAMERTAG,DISCORD,XBOX,MCC,XLAN,HPC,HOST,NOTES) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"))) {
					echo "Prepare failed: (" . $conn->errno . ") " . $conn->error;
				}

				if (!$sql->bind_param("sddssssiiiiis", $name, $lat, $lng, $city, $email, $gt, $discord, $xbox, $mcc, $xlan, $hpc, $host, $notes)) {
				...
				if (!$sql->execute()) {
				   ...
				   if (!($sql2 = $conn->prepare("SELECT ID,EMAIL,EDIT_KEY FROM FINDER.PLAYERS WHERE EMAIL=?"))) {
						echo "Prepare failed: (" . $conn->errno . ") " . $conn->error;
					}


					if (!$sql2->bind_param("s", $email)) {
						echo "Binding parameters failed: (" . $stmt->errno . ") " . $stmt->error;
					}
					...
					if (!$sql2->execute()) {
						...
						if( $retval == true ) {
							...
							echo 'alert("New player added successfully!  You will have to verify your email before you appear on the map. Please check your email, including your spam folder, for further instructions.")';
							echo '</script>';
						}
						else {
						echo "Message failed.";
